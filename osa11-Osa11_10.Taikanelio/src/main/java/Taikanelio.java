...
public class Taikanelio {
    ...
    public ArrayList<Integer> rivienSummat() {
        int koko = nelio.length;
        ArrayList<Integer> palautettava = new ArrayList<>();

        for (int rivi = 0; rivi < koko; rivi++) {
            int summa = nelio[rivi][0];
            for (int sarake = 1; sarake < koko; sarake++) {
                summa += nelio[rivi][sarake];
            }
            palautettava.add(summa);
        }
        return palautettava;
    }
    ...
    public ArrayList<Integer> sarakkeidenSummat() {
        ...
        for (int sarake = 0; sarake < koko; sarake++) {
            int summa = nelio[0][sarake];
            for (int rivi = 1; rivi < koko; rivi++) {
                summa += nelio[rivi][sarake];
            }
            palautettava.add(summa);
        }
